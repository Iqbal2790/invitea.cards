...
import { DUMMY_BIRTHDAY_DATA } from "@/lib/dummy-data";
...
export default function GreetingMinimalistTemplate() {
  // Slide 0: Candle, Slide 1: Letter, Slide 2: Memories, Slide 3: Wishes, Slide 4: Final
  ...
  const handleBlown = () => {
    ...
    // Transisi ke slide surat setelah lilin ditiup
    setTimeout(() => {
      setCurrentSlide(1);
    }, 1500); // Jeda sejenak untuk membiarkan asap lilin terlihat
  ...
  // Variant animasi untuk perpindahan antar slide (geser dari kanan ke kiri)
  const slideVariants = {
  ...
  return (
    <>
      ...
      <main className="bg-[#0A0914] h-screen w-full text-[#F8FAFC] overflow-hidden flex justify-center">
        <div className="w-full max-w-md h-full bg-gradient-to-b from-[#17153B] to-[#0A0914] relative shadow-2xl overflow-hidden flex flex-col">
          ...
          {/* Tombol Lanjut Global */}
          <AnimatePresence>
            {currentSlide > 0 && currentSlide < totalSlides - 1 && !slides[currentSlide].hideNextButton && (
              <motion.div 
                className="absolute bottom-8 left-0 right-0 flex justify-center z-50 pointer-events-auto"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: 20 }}
                transition={{ delay: 1 }} // Muncul agak terlambat
              >
                <button 
                  onClick={nextSlide}
